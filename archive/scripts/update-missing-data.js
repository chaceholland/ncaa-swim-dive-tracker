...
function normalizeClassYear(year) {
  ...
  if (normalized.startsWith('FR') || normalized === 'FRESHMAN' || normalized === '1' || normalized === '1ST') return 'freshman';
  if (normalized.startsWith('SO') || normalized === 'SOPHOMORE' || normalized === '2' || normalized === '2ND') return 'sophomore';
  if (normalized.startsWith('JR') || normalized === 'JUNIOR' || normalized === '3' || normalized === '3RD') return 'junior';
  if (normalized.startsWith('SR') || normalized === 'SENIOR' || normalized === '4' || normalized === '4TH') return 'senior';
...
async function scrapeTeamRoster(page, teamName, url) {
  ...
  try {
    ...
    // Filter to only men's athletes (some sites show combined roster)
...
async function getTeamId(teamName) {
  try {
    ...
    console.error(`  ✗ Error fetching team ID for ${teamName}:`, error.message);
    return null;
  }
}
...
async function insertAthlete(athlete, teamId) {
  try {
    const { data, error } = await supabase
      .from('athletes')
      .insert([{
    ...
    console.error(`    ✗ Error inserting athlete:`, error.message);
    return false;
  }
}
...
async function processTeam(browser, teamName, url) {
  console.log(`\n${'='.repeat(60)}`);
  ...
  // Scrape roster
  const athletes = await scrapeTeamRoster(browser, teamName, url);
  ...
  for (const athlete of athletes) {
    ...
    if (!existingAthlete) {
      // New athlete
      const success = await insertAthlete({
        ...athlete,
        ...
        profile_url: athlete.profile_url ? new URL(athlete.profile_url, url).href : null,
      }, teamId);
...
async function main() {
  ...
  try {
    ...
    results.forEach(r => {
      ...
      console.log(`${status} ${r.team.padEnd(20)} | Added: ${String(r.added).padStart(2)} | Updated: ${String(r.updated).padStart(2)} | Processed: ${String(r.processed).padStart(2)}`);
    });
