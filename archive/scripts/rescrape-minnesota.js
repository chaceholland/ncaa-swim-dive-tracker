  ...
  try {
    ...
    const photoUrl = await page.evaluate(() => {
      ...
      const candidates = images
        .filter(img => {
          const src = img.src || '';
          if (!src.includes('cloudfront.net') && !src.includes('gophersports.com/images/')) return false;
...
async function main() {
  console.log('\n🔧 RESCRAPING: Minnesota Men\'s Swimming & Diving\n');

  const { data: team } = await supabase.from('teams').select('id, logo_url').eq('name', 'Minnesota').single();
  ...
  await page.goto('https://gophersports.com/sports/mens-swimming-and-diving/roster', { waitUntil: 'domcontentloaded', timeout: 15000 });
