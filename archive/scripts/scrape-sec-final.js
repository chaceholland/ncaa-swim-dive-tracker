...
const TEAMS = {
  'Auburn': {
    baseUrl: 'https://auburntigers.com/sports/mens-swimming-and-diving/roster',
    useImgproxy: true
  },
  'Tennessee': {
    baseUrl: 'https://utsports.com/sports/mens-swimming-and-diving/roster'
  },
  'USC': {
    baseUrl: 'https://usctrojans.com/sports/mens-swimming-and-diving/roster'
  }
};
...
async function scrapeAthletePhoto(page, baseUrl, slug, teamName, useImgproxy = false) {
  ...
  try {
    ...
    const imgResponsePromise = page.waitForResponse(
      response => {
        ...
        const isValid = (url.includes('sidearmdev') || url.includes('cloudfront') || 
                url.includes('images/') || url.includes('sidearm') ||
                url.includes('imgproxy') || url.includes('utsports') ||
                url.includes('usctrojans') || url.includes('auburntigers')) &&
               response.status() === 200 &&
               !url.includes('logo') &&
...
async function scrapeTeam(teamName, teamData, browser) {
  ...
  for (const athlete of needPhotos) {
    ...
    const photoUrl = await scrapeAthletePhoto(page, teamData.baseUrl, slug, teamName, teamData.useImgproxy);
...
async function scrapeAllTeams() {
  console.log('Scraping Auburn, Tennessee, and USC for final photos...\n');
