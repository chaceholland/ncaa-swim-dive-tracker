  ...
  try {
    ...
    const photoUrl = await page.evaluate(() => {
      ...
      const candidates = images
        .filter(img => {
          const src = img.src || '';
          if (!src.includes('gopsusports') && !src.includes('storage.googleapis.com') && !src.includes('cloudfront.net') && !src.includes('sidearmdev.com')) return false;
          if (src.includes('logo') || src.includes('placeholder') || src.includes('sponsor')) return false;
...
async function main() {
  console.log('\n🔧 RESCRAPING: Penn State Full Roster\n');
  ...
  console.log('Scraping Penn State roster page...\n');
  await page.goto('https://gopsusports.com/sports/mens-swimming-and-diving/roster', {
  ...
  for (const rosterAthlete of rosterAthletes) {
    // Extract name from URL slug - format is /roster/player/first-last
    const namePart = rosterAthlete.profileUrl.split('/roster/player/')[1]?.replace(/\/$/, '').split('/')[0];
