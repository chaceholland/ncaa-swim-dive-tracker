...
const TEAMS = {
  'Columbia': {
    baseUrl: 'https://gocolumbialions.com/sports/mens-swimming-and-diving/roster'
  },
  'Dartmouth': {
    baseUrl: 'https://dartmouthsports.com/sports/mens-swimming-and-diving/roster'
  },
  'Indiana': {
    baseUrl: 'https://iuhoosiers.com/sports/mens-swimming-and-diving/roster'
  },
  'Southern Illinois': {
    baseUrl: 'https://siusalukis.com/sports/mens-swimming-and-diving/roster'
  },
  'Texas A&M': {
    baseUrl: 'https://12thman.com/sports/mens-swimming-and-diving/roster'
  }
};
  ...
  try {
    ...
    const imgResponsePromise = page.waitForResponse(
      response => {
        ...
        return (url.includes('sidearmdev') || url.includes('cloudfront') || 
                ...
                url.includes('gocolumbialions') || url.includes('dartmouthsports') ||
                url.includes('iuhoosiers') || url.includes('siusalukis') ||
                url.includes('12thman')) &&
               response.status() === 200 &&
               !url.includes('logo') &&
    ...
    const result = await page.evaluate(() => {
      const selectors = [
        ...
        '.player-image img',
        'img[src*="sidearm"]',
        'img[src*="gocolumbialions"]',
        'img[src*="dartmouthsports"]',
        'img[src*="iuhoosiers"]',
        'img[src*="siusalukis"]',
        'img[src*="12thman"]'
      ];
...
async function scrapeAllTeams() {
  console.log('Scraping final 5 teams (1 photo each)...\n');
