  ...
  try {
    ...
    const athletes = await page.evaluate(() => {
      ...
      const sidearmRows = document.querySelectorAll(
        ".sidearm-roster-player, .s-person-card, .sidearm-roster-player-container",
      );
      sidearmRows.forEach((row: any) => {
        const nameEl = row.querySelector(
          ".sidearm-roster-player-name, .s-person-details__personal-single-line a, h3 a, .s-person-card__content h3, .sidearm-roster-player-name a",
        );
        const classEl = row.querySelector(
          ".sidearm-roster-player-academic-year, .s-person-details__bio-stats-item, .academic-year, .sidearm-roster-player-academic-year abbr",
        );
        const hometownEl = row.querySelector(
          ".sidearm-roster-player-hometown, .hometown, .s-person-details__bio-stats span",
        );
        const positionEl = row.querySelector(
          '.sidearm-roster-player-position, .position, [class*="position"], .sidearm-roster-player-event',
        );
        ...
        if (name && name.length > 2) {
          ...
          let mappedClass = "";
          if (classYear.includes("fr") || classYear.includes("freshman"))
            mappedClass = "freshman";
          else if (classYear.includes("so") || classYear.includes("sophomore"))
            mappedClass = "sophomore";
          else if (classYear.includes("jr") || classYear.includes("junior"))
            mappedClass = "junior";
          else if (classYear.includes("sr") || classYear.includes("senior"))
            mappedClass = "senior";
      ...
      if (results.length === 0) {
        const tableRows = document.querySelectorAll(
          "table.roster tbody tr, table.sidearm-table tbody tr, .roster-table tbody tr",
        );
      ...
      if (results.length === 0) {
        const cards = document.querySelectorAll(
          '.roster-card, .athlete-card, .player-card, [class*="roster"] [class*="card"]',
        );
        cards.forEach((card: any) => {
          const nameEl = card.querySelector(
            'h3, h4, .name, .player-name, a[href*="player"]',
          );
          const name = nameEl?.textContent?.trim();

          if (name && name.length > 2) {
            const classEl = card.querySelector(
              '.class, .year, .academic-year, [class*="year"]',
            );
            const classYear = classEl?.textContent?.trim()?.toLowerCase() || "";
      ...
      if (results.length === 0) {
        const listItems = document.querySelectorAll(
          'li.player, li.athlete, li[class*="roster"], ul.roster li',
        );
      ...
      if (results.length === 0) {
        const playerLinks = document.querySelectorAll(
          'a[href*="player"], a[href*="athlete"], a[href*="roster"]',
        );
    ...
    if (athletes.length === 0) {
      console.log(
        `  ⚠️  No athletes found - may not have men's program or different site structure`,
      );
      await page.close();
    ...
    console.log(
      `  💾 Inserted ${successCount}/${athletes.length} athletes into database`,
    );
    await page.close();
...
async function scrapeAllRosters() {
  ...
  const hasErrors = failedTeams > 0;
  try {
    const { error: syncLogError } = await supabase
      .from("swim_sync_log")
      .insert({
        sync_type: "roster_scrape",
        source: "scrape-athletes-v2",
        records_count: totalAthletes,
        status: hasErrors ? "error" : "success",
        error_message: hasErrors
          ? `${failedTeams} team(s) failed out of ${rosterUrls.length}`
          : null,
      });
    if (syncLogError) {
