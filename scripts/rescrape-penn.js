  ...
  try {
    ...
    const photoUrl = await page.evaluate(() => {
      ...
      const candidates = images
        .filter(img => {
          const src = img.src || '';
          if (!src.includes('pennathletics.com') && !src.includes('cloudfront.net') && !src.includes('sidearmdev.com')) return false;
          if (src.includes('logo') || src.includes('placeholder') || src.includes('responsive_')) return false;
...
async function main() {
  ...
  console.log('Scraping Penn roster page...\n');
  await page.goto('https://pennathletics.com/sports/mens-swimming-and-diving/roster', {
