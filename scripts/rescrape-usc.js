  ...
  try {
    ...
    const photoUrl = await page.evaluate(() => {
      ...
      const candidates = images
        .filter(img => {
          const src = img.src || '';
          if (!src.includes('cloudfront.net') && !src.includes('usctrojans.com/images/')) return false;
...
async function main() {
  console.log('\n🔧 RESCRAPING: USC Men\'s Swimming & Diving\n');

  const { data: team } = await supabase.from('teams').select('id, logo_url').eq('name', 'USC').single();
  ...
  await page.goto('https://usctrojans.com/sports/mens-swimming-and-diving/roster', { waitUntil: 'domcontentloaded', timeout: 15000 });
