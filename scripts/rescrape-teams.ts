...
async function scrapeTeam(teamName: string, rosterUrl: string, teamId: string) {
  ...
  try {
    ...
    for (const pattern of patterns) {
        ...
        for (const item of items) {
          const isInMensSection = await item.evaluate((el) => {
            ...
            for (let i = 0; i < headings.length; i++) {
              ...
              if (
                lowerText === "men's roster" ||
                lowerText === "men's team" ||
                (lowerText.includes("men's roster") &&
            ...
            const afterMens =
              (mensHeading.compareDocumentPosition(el) &
                Node.DOCUMENT_POSITION_FOLLOWING) !==
              0;

            // Check if el comes before next heading
            const beforeNext =
              !nextHeadingAfterMens ||
              (el.compareDocumentPosition(nextHeadingAfterMens) &
                Node.DOCUMENT_POSITION_FOLLOWING) !==
                0;
      ...
      console.log(
        `  ✓ Found ${items.length} athletes using pattern "${pattern.container}"`,
      );
      ...
      for (const item of items) {
        try {
          ...
          if (photoElement) {
            ...
            if (!photoSrc)
              photoSrc = await photoElement.getAttribute("data-src");
            if (photoSrc) {
          ...
          if ((pattern as any).getMensOnly && pattern.position === "strong") {
            ...
            const texts = await Promise.all(
              strongElements.map((el) => el.textContent()),
            );
            position = texts.join(" ");
          } else {
          ...
          if (position) {
            ...
            if (positionLower.includes("div")) {
              ...
              positionLower.match(
                /free|breast|back|fly|butterfly|im|sprint|distance|medley|stroke/,
              )
            ) {
          ...
          if (year) {
            ...
            if (yearText.match(/freshman|fr|1st/))
              athlete.class_year = "freshman";
            else if (yearText.match(/sophomore|so|2nd/))
              athlete.class_year = "sophomore";
            else if (yearText.match(/junior|jr|3rd/))
              athlete.class_year = "junior";
            else if (yearText.match(/senior|sr|4th|fifth|5th|grad/))
              athlete.class_year = "senior";
          }
    ...
    if (error) {
      ...
      console.log(
        `  💾 Inserted ${data?.length}/${athletes.length} athletes into database`,
      );
    }
  } catch (error: any) {
...
async function main() {
  ...
  console.log(
    `📊 Teams to scrape: ${teamsToScrape.map((t) => t.name).join(", ")}\n`,
  );
  ...
  const totalProcessed = teamsToScrape.length;
  try {
    const { error: syncLogError } = await supabase
      .from("swim_sync_log")
      .insert({
        sync_type: "roster_rescrape",
        source: "rescrape-teams",
